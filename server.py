        ...
        if slope_to_center is not None and timer == 0.0:
            ...
            if timer != 0.0:
                if time.monotonic() - timer >= 0.0 and time.monotonic() - timer < 0.2:
                    ...
                    print("dir : Stop22")                
                    timer = 0.0
            else: # 차선 검출을 못해서 기울기가 None이면
                # if backward_count >= 3: # 후진 횟수가 3회 이상이면
                #     if pre_command == 0b00100000:   # 이전 명령이 전진이면 후진
                #         command = 0b01000000
                #     elif pre_command == 0b11000000: # 이전 명령이 좌회전이면 우회전
                #         command = 0b11100000
                #         print("dir : Right2")
                #     elif pre_command == 0b11100000: # 이전 명령이 우회전이면 좌회전
                #         command = 0b11000000
                #         print("dir : Left2")
                command = 0b01000000
                print("dir : backward2")
